...
class Dataset_json(object):
    def __init__(self, dataset_name=None,  label_percent=0.1 ,go_backwards=False, pad_mode=PadMode.POST):
        # Public properties
        ...
        self.labeled_indices = np.random.choice(self.anomaly_indices, size=int(
            len(self.anomaly_indices) * self.label_percent),replace=False)  ### 半监督学习中，知道的标签位置
